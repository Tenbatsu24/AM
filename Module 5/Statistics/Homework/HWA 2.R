...
# **Q**  
# g)  Read the data set in the supplementary file athletes.csv into the statistical software R. Estimate the probability in part e. by replacing ?? and ?? by the corresponding MLE and compute the confidence intervals in part f. (The best would be if you print the R script and attach it to your homework).
...
# Estimate the probability in part e. by replacing ?? and ?? by the corresponding MLE
prob = 1 - pnorm(4, mean = mu_mle, sd = sigma_mle)
sprintf(" The probability a 'clean' athlete has suspicious ratio is= %f", prob)

# compute the confidence intervals in part f for mu_mle
c_0.025 = qt(1-0.025, df=n-1)
s = sd(TEratios)
mu_mle_ci_0.025 = c(mu_mle - c_0.025*(s)/sqrt(n), mu_mle + c_0.025*(s)/sqrt(n))
...
sigma_mle_ci_0.05 = c(sqrt((n-1)*(s^2)/c_0.05_1), sqrt((n-1)*(s^2)/c_0.05_2))
